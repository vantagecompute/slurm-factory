...
from slurm_factory.utilities import SLURM_VERSIONS
...
class TestGenerateSpackConfig:
    ...
    def test_slurm_external_dependencies(self):
        ...
        for spack_name, (ubuntu_pkg, version) in expected_externals.items():
            ...
            assert pkg_config["buildable"] is False, (
                f"{spack_name} should not be buildable (uses {ubuntu_pkg})"
            )
...
class TestGetCommentHeader:
    ...
    def test_get_comment_header_cpu_only(self):
        ...
        assert (
            header == "# Spack environment for building Slurm 23.11 (optimized for minimal runtime footprint)"
        )
...
class TestGenerateYamlString:
    ...
    def test_generate_yaml_string_formatting(self):
        ...
        specs_line_idx = (
            next(i for i, line in enumerate(lines[spack_line_idx:]) if line.strip().startswith("specs:"))
            + spack_line_idx
        )
