...
class TestSlurmEnvironmentConfig:
    ...
    def test_slurm_env_gcc_requirements_match_bootstrap(self):
        """Test that Slurm env GCC requirements match compiler bootstrap."""
        from slurm_factory.spack_yaml import get_gcc_buildcache_requirements
        
        compiler_version = "13.4.0"
        
        # Get requirements from Slurm environment config
        slurm_config = generate_spack_config(compiler_version=compiler_version)
        gcc_package_config = slurm_config["spack"]["packages"]["gcc"]
        slurm_requirements = gcc_package_config["require"]
        
        # Get requirements that should match
        expected_requirements = get_gcc_buildcache_requirements(compiler_version)
        
        # They should be identical
        assert set(slurm_requirements) == set(expected_requirements), \
            f"GCC requirements mismatch: {slurm_requirements} != {expected_requirements}"
            
    def test_slurm_env_prevents_system_gcc(self):
        """Test that Slurm environment prevents use of system GCC."""
        config = generate_spack_config()
        
        # GCC package should have empty externals
        gcc_config = config["spack"]["packages"]["gcc"]
        assert gcc_config["externals"] == []
        assert gcc_config["buildable"] is True
...
class TestDockerfileBuildScript:
    ...
    def test_build_script_switches_to_slurm_environment(self):
        ...
        assert "spack -e . install --only-concrete --cache-only" in script
    ...
    def test_build_script_compiler_verification(self):
        ...
        # Should verify exactly one compiler is available
        assert "COMPILER_COUNT=" in script
        assert "Expected exactly 1 gcc compiler" in script
        assert "gcc@13.4.0" in script
...
class TestCompilerBootstrapIntegration:
    ...
    def test_compiler_and_slurm_yaml_compatibility(self):
        ...
        slurm_gcc_reqs = slurm_config["spack"]["packages"]["gcc"]["require"]
        assert f"@{compiler_version}" in slurm_gcc_reqs
        
        # Both should prevent system GCC
        assert compiler_config["spack"]["packages"]["gcc"]["externals"] == []
        assert slurm_config["spack"]["packages"]["gcc"]["externals"] == []
