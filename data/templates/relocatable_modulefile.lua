...
if not slurm_conf then
    ...
    local default_conf = pathJoin(slurm_prefix, "etc/slurm/slurm.conf")
    local sysconfdir_conf = "/etc/slurm/slurm.conf"
    
    if isFile(sysconfdir_conf) then
        setenv("SLURM_CONF", sysconfdir_conf)
    elseif isFile(default_conf) then
