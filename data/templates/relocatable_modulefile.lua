...
-- -*- lua -*-
-- Enhanced module file for Slurm workload manager
-- Generated by Spack with advanced Spack 1.x features
{% if spec -%}
-- {{ spec.short_spec }}
{% else -%}
...
{% endif -%}
help([[
Slurm Workload Manager - MPI Tier Package
...
Module Hierarchy:
  Core -> Compiler -> MPI (this level)

Relocatability Features:
- Binaries use RPATH for library discovery (Spack 1.x feature)
- Falls back to LD_LIBRARY_PATH for incomplete RPATH scenarios  
- Can be relocated by setting SLURM_INSTALL_PREFIX before loading
- Self-contained installation with all runtime dependencies
- Compatible with containerized and distributed deployments
- Built with enhanced compiler packaging (gcc-runtime)

Advanced Features:
- Binary cache support for faster rebuilds
- Enhanced RPATH/RUNPATH configuration
- Proper module hierarchy with automatic dependency loading
- Professional module templates with conflict resolution
...
Dependencies:
  This module may automatically load required Core and Compiler modules
  depending on the hierarchy configuration.

Note: For optimal performance, binaries use RPATH for relocatability.
Binary cache (buildcache) can be enabled for faster rebuilds.
]])
