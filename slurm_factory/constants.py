...
def get_spack_build_script(compiler_version: str) -> str:
    ...
    return textwrap.dedent(f"""        source {SPACK_SETUP_SCRIPT}
        ...
        echo '==> Ensuring custom repo is available...'
        # The repos section in spack.yaml should trigger automatic cloning
        # Force environment refresh to ensure repos are loaded
        spack env status
        ...
        spack list slurm_factory.slurm || {{
            ...
            spack repo list
            echo 'Attempting to manually add repo...'
            if [ ! -d /root/slurm-factory-spack-repo ]; then
                git clone -b main \\
...
def get_package_tarball_script(
    ...
    return textwrap.dedent(f"""\
        ...
        {gpu_handling_script if gpu_handling_script else ""}find . -name "include" -type d -exec rm -rf {{}} + 2>/dev/null || true && \\
