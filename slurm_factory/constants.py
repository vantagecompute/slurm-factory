...
COMPILER_TOOLCHAINS = {
    "13.3.0": ("13.3.0", "2.39", "Ubuntu 24.04 (default) - glibc 2.39"),
    "11.4.0": ("11.4.0", "2.35", "Ubuntu 22.04 compatible - glibc 2.35"),
    "10.5.0": ("10.5.0", "2.31", "RHEL 8 / Ubuntu 20.04 compatible - glibc 2.31"),
...
def get_dockerfile(
    ...
    Stage 0 (compiler-bootstrap): Build custom GCC (only when use_spack_compiler=True)
    ...
    Args:
        ...
        use_spack_compiler: Whether to build a custom GCC toolchain
...
"""
    ...
    # Choose base stage: either compiler-bootstrap or ubuntu:24.04
    base_stage = "compiler-bootstrap" if use_spack_compiler else "ubuntu:24.04"

    # Only install deps/spack in init stage if we didn't already in compiler-bootstrap
    init_stage_setup = ""
    if not use_spack_compiler:
        init_stage_setup = textwrap.dedent(
            f"""\
ENV DEBIAN_FRONTEND=noninteractive
ENV TZ=UTC
